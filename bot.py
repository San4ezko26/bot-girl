...
def main_message(message):
	...
	if message.text == b0:
		...
		wrk0 = types.InlineKeyboardButton(text="Чат воркеров", url=f"{chat_workers}")
		wrk1 = types.InlineKeyboardButton(text="Мануал для работы", url=f"{text_workers}")
		wrk2 = types.InlineKeyboardButton(text="Создать промокод", callback_data="prom")
		wrk3 = types.InlineKeyboardButton(text="Закрыть", callback_data="menu")

		wrk.add(wrk0, wrk1)
		wrk.add(wrk2)
		wrk.add(wrk3)
		bot.send_message(message.chat.id,f"[Ваша реферальная ссылка](http://t.me/{bot_username}?start={message.chat.id})", parse_mode='Markdown', reply_markup=wrk)
		...
		if call.data == "next":
			...
			if spso == 1:
				...
				try:
					...
					for x in range(len(req['data'])):
						...
						if req['data'][x]['comment'] == comment:
							...
							bot.send_message(wk,f"🐘 Ваш мамонт: [{call.message.chat.first_name}](tg://user?id={call.message.chat.id}) пополнил баланс на {skolko}RUB",parse_mode='Markdown')
				...
				try:
					...
					k.add(k1)
					k.add(k2)
					bot.edit_message_text(chat_id=call.message.chat.id, message_id=call.message.message_id,text ="Ваш платеж проверяется,подождите.")
			...
			bot.send_message(call.message.chat.id,"Отправьте айди мамонта и Сообщение в формате id:Сообщение\n\nНапример  123456789:Ты мамонт",reply_markup=cancel())
	...
	try:
		...
		if testpromo == b20:
			...
			if r == 0:
				...
				bot.send_message(message.chat.id,f"♻️ Ваш баланс пополнен на {summpromo}\n💰 Баланс {balancenow+summpromo} RUB",reply_markup=userpanel())
				bot.register_next_step_handler(message, main_message)
...
def balik(message):
	if message.text == b12:
		bot.send_message(message.chat.id,"Укажите сумму, на которую хотите пополнить кошелёк.",reply_markup=cancel())
		...
		if message.text.isdigit():
			...
			if skolko >= minimalka and skolko <=maximalka:
				...
				if spso == 1:
					...
					texttt = f'*♻️ Оплата QIWI/банковской картой:* [ОПЛАТА](https://qiwi.com/n/{qiwi_nick})\n\nСумма: *{skolko}₽*\nКомментарий: "{comment}"\n\nВАЖНО! Обязательно пишите комментарий к платежу!\nЕсли вы не укажете комментарий, деньги не поступят на счет!'
				...
				bot.send_message(message.chat.id,f"Минимальная сумма пополнения - {minimalka} RUB")
				bot.register_next_step_handler(message, popolni)
			...
			if skch.isdigit():
				if int(skch) >= 0 and int(skch) <=24:
					if int(skch)%1 == 0:
						...
						if op > bnow:
							bot.send_message(message.from_user.id, f"На балансе недостатачно средств.\nСумма заказа {op}\nНа балансе {bnow}",reply_markup=bal())
							bot.register_next_step_handler(message, balik)
						else:
							con = sqlite3.connect("data.db")
							cur = con.cursor()
